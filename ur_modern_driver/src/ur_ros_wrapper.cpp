...
private:
	...
	void publishRTMsg() {
      ...
      //Create quaternion
      tf::Quaternion quat;
      double x = tool_vector_actual[0];
      double y = tool_vector_actual[1];
      double z = tool_vector_actual[2];
      ...
      // make sur to broadcast only if no nans
      if (!std::isnan(x) && !std::isnan(y) && !std::isnan(z) && !std::isnan(rx/angle) && !std::isnan(ry/angle) && !std::isnan(rz/angle) && std::abs(x)<100 && std::abs(y)<100 && std::abs(z)<100) {
        if (std::abs(angle)<1e-6 || std::isnan(angle))
          {
        ...
        print_warning("Nan in the tool0_controller transform...");
      }
